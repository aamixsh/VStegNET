...
def revealing_network(container):
    with tf.variable_scope('reveal_net'):
        ...
        conv1 = tf.layers.conv3d(container,32,3,padding='same',name="1",activation=tf.nn.relu,data_format='channels_last')  
       ...
       # upsample1 = conv3dTranspose(filters=256,kernel_size=3, padding='same', activation=tf.nn.relu,data_format='channels_last', data_format='channels_last')(upsample1)
        ...
        upsample4 = tf.keras.layers.UpSampling3D(size=(2,2,2),  data_format='channels_last')(conv8)
        #print(upsample4)
...
container = tf.placeholder(shape=input_shape, dtype=tf.float32, name='container_input')
